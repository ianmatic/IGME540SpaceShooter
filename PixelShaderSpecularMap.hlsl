...
float4 ComputeDirectionalLight(DirectionalLight light, float3 normal, float3 toCamera, float4 surfaceColor, float specularMap) {
	...
	result += specularAmount * specularMap;

	return result;
}

// Helper method that computes point lights
float4 ComputePointLight(PointLight light, float3 normal, float3 worldPos, float3 toCamera, float4 surfaceColor, float specularMap) {
	// Diffuse calculation
...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	float specularMap = specularTexture.Sample(basicSampler, input.uv).r;
	...
	return
		ComputeDirectionalLight(light, input.normal, input.worldPos, surfaceColor, specularMap) +
		ComputeDirectionalLight(secondLight, input.normal, input.worldPos, surfaceColor, specularMap) +
		ComputePointLight(pointLight, input.normal, input.worldPos, toCamera, surfaceColor, specularMap) + 
		ComputePointLight(secondPointLight, input.normal, input.worldPos, toCamera, surfaceColor, specularMap);
}